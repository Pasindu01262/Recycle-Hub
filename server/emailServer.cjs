...
app.post("/send-email", async (req, res, next) => {
  try {
    ...
    console.log("Received form data:", { first_name, last_name, user_email, message: message?.substring(0, 50) + "..." });
    
    if (!first_name || !user_email || !message) {
      return res.status(400).json({ error: "Missing required fields: first_name, user_email, and message are required" });
    }
    ...
    console.log("Sending email to:", process.env.CONTACT_FORM_TO_EMAIL);
